...
THREE.Director.prototype.applyTweens = function(currentTime, lastTime) {
	...
	for (i=0, il=tweens.length; iil; i++) {  Quick patch for ordering
		tween = tweens[i];
...
THREE.Director.prototype.merge = function(director2, time) {
	...
	for (i=0,il=actions.length; iil; i++) {
		actions[i].time += time;
	}

	for (i=0,il=tweens.length; iil; i++) {
